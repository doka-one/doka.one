...
pub(crate) fn test_db_connection(config: &Config) -> anyhow::Result<()> {
    ...
    let url = format!(
        "postgresql://{}:{}@{}:{}/{}",
        &config.db_user_name, &config.db_user_password, &config.db_host, &config.db_port, SYSTEM_DATABASE
    );
...
pub(crate) fn create_doka_user(config: &Config) -> anyhow::Result<()> {
    ...
    let mut cnx =
        Client::connect(&url, NoTls).map_err(eprint_fwd!("Cannot connect the database: {}", SYSTEM_DATABASE))?;

    cnx.batch_execute(&sql).map_err(eprint_fwd!("Create doka user error"))?;
...
fn add_db_routine(config: &Config, db_name: &str) -> anyhow::Result<()> {
    ...
    let url = format!(
        "postgresql://{}:{}@{}:{}/{}",
        &config.db_user_name, &config.db_user_password, &config.db_host, &config.db_port, db_name
    );
    ...
    cnx.batch_execute(SQL_EXTRA_ROUTINES).map_err(eprint_fwd!("add db routine script error"))?;
