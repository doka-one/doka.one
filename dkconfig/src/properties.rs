...
pub fn get_prop_value(prop_name : &str) -> anyhow::Result<String> {
    ...
    // let s = PROPS.read().unwrap().deref().get(&0).unwrap().deref()
...
pub fn get_prop_pg_connect_string() -> anyhow::Result<(String,u32)> {
    let db_hostname = get_prop_value("db.hostname").map_err(err_fwd!(""))?;
    let db_port = get_prop_value("db.port").map_err(err_fwd!(""))?;
    let db_name = get_prop_value("db.name").map_err(err_fwd!(""))?;
    let db_user = get_prop_value("db.user").map_err(err_fwd!(""))?;
    let db_password = get_prop_value("db.password").map_err(err_fwd!(""))?;
