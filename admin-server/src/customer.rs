...
use dkdto::error_codes::{
    ...
    INVALID_PASSWORD, INVALID_TOKEN, USER_NAME_ALREADY_TAKEN,
};
use dkdto::web_types::{
    AddKeyRequest, CreateCustomerReply, CreateCustomerRequest, SimpleMessage, WebType, WebTypeBuilder,
};
...
async fn set_removable_flag_customer_from_db(
    ...
    let _nb = query.update(trans).await.map_err(err_fwd!("Set removable flag for customer failed"))?;
...
impl CustomerDelegate {
    ...
    pub async fn create_customer(
        ...
        if !valid_password(&customer_request.admin_password) {
            log_error!("💣 Password breaks the syntax rules, follower=[{}]", &self.follower);
        ...
        if self.check_customer_name_not_taken(&mut trans, &customer_request.customer_name).await.is_err() {
        ...
        if self.check_user_name_not_taken(&mut trans, &customer_request.email).await.is_err() {
            log_error!("The customer name is already taken, follower=[{}]", &self.follower);
        ...
        loop {
            ...
            let Ok(not_taken) = self
                ...
                .await
                .map_err(err_fwd!("Cannot verify the customer code uniqueness, follower=[{}]", &self.follower))
            else {
                return WebType::from_api_error(&INTERNAL_DATABASE_ERROR);
            };
        ...
        let add_key_request = AddKeyRequest { customer_code: customer_code.clone() };

        let Ok(km_host) =
            get_prop_value(KEY_MANAGER_HOSTNAME_PROPERTY).map_err(err_fwd!("Cannot read the key manager hostname"))
        else {
        ...
        let response = kmc.add_key(&add_key_request, &self.follower.token_type).await;
        ...
        log_info!("😎 Key manager success, customer=[{}], follower=[{}]", &customer_code, &self.follower);
        ...
        let sql_insert = SQLChangeAsync {
            sql_query: r#"INSERT INTO dokaadmin.customer (code, full_name, default_language, default_time_zone)
                        VALUES (:p_code, :p_full_name, :p_default_language, :p_default_time_zone) "#
                .to_string(),
            params,
            sequence_name: "dokaadmin.customer_id_seq".to_string(),
        };

        let Ok(customer_id) = sql_insert
            .insert(&mut trans)
            .await
            .map_err(err_fwd!("💣 Insertion of a new customer failed, follower=[{}]", &self.follower))
        else {
            let _ = warning_cs_schema(&customer_code);
            let _ = warning_fs_schema(&customer_code);
            return WebType::from_api_error(&INTERNAL_DATABASE_ERROR);
        ...
        WebType::from_item(
            ...
            CreateCustomerReply { customer_code, customer_id, admin_user_id: user_id },
        )
    }
    ...
    pub async fn delete_customer(&mut self, customer_code: &str) -> WebType<SimpleMessage> {
            ...
            Ok((_customer_id, is_removable)) => {
                if is_removable {
                    ...
                    if self
                        ...
                        .await
                        .map_err(err_fwd!("💣 Cannot delete user, follower=[{}]", &self.follower))
                        .is_err()
                    {
                        return WebType::from_api_error(&INTERNAL_DATABASE_ERROR);
                    }
                    ...
                    if self
                        ...
                        .await
                        .map_err(err_fwd!("💣 Cannot delete customer, follower=[{}]", &self.follower))
                        .is_err()
                    {
                        return WebType::from_api_error(&INTERNAL_DATABASE_ERROR);
                    }
        ...
        if self
            ...
            .await
            .map_err(err_fwd!("💣 Cannot delete the CS schema, follower=[{}]", &self.follower))
            .is_err()
        {
            trans.rollback().await;
            return WebType::from_api_error(&INTERNAL_DATABASE_ERROR);
        ...
        if self
            ...
            .await
            .map_err(err_fwd!("💣 Cannot delete the FS schema, follower=[{}]", &self.follower))
            .is_err()
        {
            trans.rollback().await;
            return WebType::from_api_error(&INTERNAL_DATABASE_ERROR);
    ...
    async fn run_script(&self, dbi: DbServerInfo, batch_script: &str, title: &str) -> anyhow::Result<()> {
        // Open a transaction on the cs database
        let connect_string =
            format!("postgresql://{}:{}@{}:{}/{}", dbi.db_user, dbi.password, dbi.host, dbi.port, dbi.db_name);
        ...
        let _ = process.batch(&mut trans).await.map_err(err_fwd!("{} batch script error", &title));
        ...
        log_info!("Finished db script for [{}] schema, follower=[{}]", &title, &self.follower);
    ...
    async fn run_fs_script(&self, customer_code: &str) -> anyhow::Result<()> {
        let dbi = DbServerInfo::for_fs();
        self.run_script(dbi, &generate_fs_schema_script(customer_code), "FS").await
    }

    async fn run_cs_script(&self, customer_code: &str) -> anyhow::Result<()> {
        let dbi = DbServerInfo::for_cs();
        self.run_script(dbi, &generate_cs_schema_script(customer_code), "CS").await
    }
    ...
    async fn search_customer(
        ...
        let mut data_set = query.execute(trans).await.map_err(err_fwd!("Query failed"))?;
    ...
    async fn drop_cs_schema_from_db(&self, customer_code: &str) -> anyhow::Result<bool> {
        let dbi = DbServerInfo::for_cs();
        self.run_script(dbi, &format!(r"DROP SCHEMA cs_{} CASCADE", customer_code), "DROP_CS").await?;
        Ok(true)
    }

    async fn drop_fs_schema_from_db(&self, customer_code: &str) -> anyhow::Result<bool> {
        let dbi = DbServerInfo::for_fs();
        self.run_script(dbi, &format!(r"DROP SCHEMA fs_{} CASCADE", customer_code), "DROP_FS").await?;
        Ok(true)
    }
    ...
    async fn delete_customer_from_db(
        ...
        let query = SQLChangeAsync {
            sql_query: r"DELETE FROM dokaadmin.customer WHERE code = :p_customer_code AND is_removable = TRUE"
                .to_string(),
            params,
            sequence_name: "".to_string(),
        };
        let _nb = query.delete(trans).await.map_err(err_fwd!("Delete of customer failed"))?;
    ...
    pub async fn delete_integration_tests_customer(mut self) -> WebType<SimpleMessage> {
        ...
        log_info!("🚀 Start delete_integration_tests_customer api, follower=[{}]", &self.follower);
        ...
        let mut params = HashMap::new();
        params.insert("p_customer_name".to_owned(), CellValue::from_raw_string("doo_%".to_string()));
        let query = SQLQueryBlockAsync {
        ...
        while sql_result.next() {
            let customer_code = sql_result
                .get_string("code")
                .ok_or_else(|| {
                    log_error!("💣 Cannot find the customer code in the result, follower=[{}]", &self.follower);
                    anyhow::anyhow!("Customer code not found")
                })
                .unwrap();

            log_info!("Found integration tests customer code=[{}], follower=[{}]", customer_code, &self.follower);
        ...
        log_info!("🏁 End delete_integration_tests_customer, follower=[{}]", &self.follower);
    ...
    ///
    pub async fn set_removable_flag_customer(mut self, customer_code: &str) -> WebType<SimpleMessage> {
        log_info!(
        ...
        if set_removable_flag_customer_from_db(&mut trans, &customer_code)
            .await
            .map_err(err_fwd!("💣 Cannot set the removable flag, follower=[{}]", &self.follower))
            .is_err()
        {
            return WebType::from_api_error(&INTERNAL_DATABASE_ERROR);
        }
