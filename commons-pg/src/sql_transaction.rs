...
pub struct SQLDataSet {
    ...
    pub position: usize,
    pub data: Box<Vec<HashMap<String, CellValue>>>,
}
...
impl CellValue {
    ...
    pub fn from_opt_str(option_val: Option<&str>) -> Self {
        match option_val {
            ...
            Some(val) => CellValue::from_raw_string(val.to_owned()),
        }
    }

    pub fn from_opt_systemtime(option_val: Option<SystemTime>) -> Self {
        match option_val {
            None => CellValue::SystemTime(None),
            Some(val) => CellValue::from_raw_systemtime(val),
        }
    }
