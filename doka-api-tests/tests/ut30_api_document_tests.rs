...
const TEST_TO_RUN : &[&str] = &["t10_create_document", "t20_create_document_with_props", "t30_add_props", "t40_delete_props"];

#[cfg(test)]
mod api_document_tests {
    use anyhow::anyhow;
    use dkdto::{AddItemRequest, AddItemTagRequest, AddTagValue, EnumTagValue, ErrorMessage, GetItemReply, LoginRequest, TagValueElement};
    ...
    ///
    /// Create item and delete tags
    ///
    #[test]
    fn t40_delete_props() -> Result<(), ErrorMessage>  {
        let lookup = Lookup::new("t40_delete_props", TEST_TO_RUN); // auto dropping
    //
    //     // Login
    //     let test_env = read_test_env();
    //
    ...
    //     // Delete props
    //
    //     document_server.delete_item_tag();
    //
    //     // Read the item
    //     let get_item_reply = document_server.get_item(item_reply.item_id, &login_reply.session_id)?;
    //     let item_name_back = get_item_reply.items.get(0).unwrap().name.clone();
