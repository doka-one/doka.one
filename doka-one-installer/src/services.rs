...
use crate::{step_println, Config, DEF_FILE_TEMPLATE};
...
fn create_service(config: &Config, service_id: &str) -> anyhow::Result<()> {
    ...
    let _the_output = Command::new(serman_program.as_str())
        .args(&["install", &service_definition_file, "--overwrite"])
        .output()
...
fn write_service_definition_file_for_tika(config: &Config) -> anyhow::Result<()> {
    ...
    let tika_config_path = format!(
        "{}/doka-configs/{}/{}/config/tika-config.xml",
        &config.installation_path, &config.instance_name, service_id
    );
